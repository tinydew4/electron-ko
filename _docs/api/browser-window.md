...
[Frameless 윈도우](http://electron.atom.io/docs/api/frameless-window) API를 사용할 수 있습니다.
...
* `icon` [NativeImage](http://electron.atom.io/docs/api/native-image) - 윈도우 아이콘, 생략하면 실행 파일의
  아이콘이 대신 사용됩니다.
* `icon` [NativeImage](http://electron.atom.io/docs/api/native-image) - 윈도우 아이콘. Windows에선 가장 좋은
...
* `frame` Boolean - `false`로 지정하면 창을 [Frameless Window](http://electron.atom.io/docs/api/frameless-window)
...
* `transparent` Boolean - 윈도우를 [투명화](http://electron.atom.io/docs/api/frameless-window)합니다. 기본값은
  `false`입니다.
...
* `preload` String - 스크립트를 지정하면 페이지 내의 다른 스크립트가 작동하기 전에
  ...
  [여기](http://electron.atom.io/docs/api/process#event-loaded)의 예시를 참고하세요.
* `session` [Session](http://electron.atom.io/docs/api/session#class-session) - 페이지에서 사용할 세션을
...
메서드나 이벤트에 대한 자세한 내용은 [`webContents` 문서](http://electron.atom.io/docs/api/web-contents)를
참고하세요.
...
* `overlay` [NativeImage](http://electron.atom.io/docs/api/native-image) - 작업표시줄 아이콘의 우측 하단에 표시될
...
* `Button` 객체
  * `icon` [NativeImage](http://electron.atom.io/docs/api/native-image) - 미리보기 툴바에 보여질 아이콘.
