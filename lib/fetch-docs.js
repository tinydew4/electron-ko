...
var fetchDocs = module.exports = function fetchDocs (_settings, callback) {
  ...
  request.get(opts, function (err, resp, body) {
    ...
    tarballUrl = 'https://github.com/tinydew4/electron/archive/translate-ko.tar.gz'
    version = 'v1.4.1'
...
function extractDocs (callback) {
  ...
  var extract = tar.extract(settings.tmpDir, {
    ignore: function (name) {
      if (name.match('docs-translations/ko-KR/api')) return false
      if (name.match('docs-translations/ko-KR/development')) return false
      if (name.match('docs-translations/ko-KR/tutorial')) return false
...
function constructDocMetadata (filepath) {
  ...
  var metadata = {
    version: version,
    category: toTitleCase(pathArray[pathArray.length - 1]).replace(/Api/, 'API').replace(/Faq/, 'FAQ'),
    redirect_from: []
  }

  // add redirect for old filename
  if (pathArray === 'locales.md') metadata.redirect_from = ['/docs-translations/ko-KR/api/app-locales/']
...
function moveDirectories (callback) {
  var sourceDir = path.join(newDir, 'docs-translations/ko-KR')
